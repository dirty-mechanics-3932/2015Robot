...
public class Elevator extends Subsystem implements Settable {
	...
	private static final int DEFAULT_MIN_TICKS = 50;
	private static final int DEFAULT_MAX_TICKS = 14100;

	public TrajectoryGenerator trajectory = new TrajectoryGenerator();
	private int minTicks = DEFAULT_MIN_TICKS;
	...
	private int safety(int setpoint) {
		if (setpoint > getMaxTicks()) {
			return getMaxTicks();
		}
		if (setpoint < getMinTicks()) {
			setpoint = getMinTicks();
		}
		return setpoint;
	}
