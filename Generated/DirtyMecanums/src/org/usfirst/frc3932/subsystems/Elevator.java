...
//    public static final int GROUND = 0; // on the ground
    ...
    private static final double PLATFORM = START + PLATFORM_HEIGHT; // 2" above start
    private static final double SCORING = PLATFORM_HEIGHT + 1d; // 3" above start
    private static final double CAN_STEP = START + 6d; // at start height with respect to a can on the step
    private static final double CAN_BOTTOM = LOW_TAB_HEIGHT - RecycleBin.BOTTOM_TO_LIP; // at the bottom of the can, below the low tabs
    private static final double WEIGHT_BEARING = LOW_TAB_HEIGHT - (2d * Tote.HEIGHT_INCHES); // two totes below the low tabs
    private static final double LOW_CAN = LOW_TAB_HEIGHT - RecycleBin.BOTTOM_TO_LIP + START; // can at the low tabs
    private static final double TABS = LOW_TAB_HEIGHT - Tote.HEIGHT_INCHES; // one tote below the tabs
    private static final double HANG = LOW_TAB_HEIGHT - Tote.LIP_RIB; // around a half inch below the tabs
    private static final double CAN = HIGH_TAB_HEIGHT - RecycleBin.BOTTOM_TO_LIP + START; // can at the cantabs
    ...
    public static enum Position {
    	START(Elevator.START),
    	TRANSPORT(Elevator.TRANSPORT),
    	PLATFORM(Elevator.PLATFORM), 
    	SCORING(Elevator.SCORING),
    	CAN_STEP(Elevator.CAN_STEP),
    	CAN_BOTTOM(Elevator.CAN_BOTTOM),
    	WEIGHT_BEARING(Elevator.WEIGHT_BEARING),
    	LOW_CAN(Elevator.LOW_CAN),
    	TABS(Elevator.TABS),
    	HANG(Elevator.HANG),
    	CAN(Elevator.CAN),
